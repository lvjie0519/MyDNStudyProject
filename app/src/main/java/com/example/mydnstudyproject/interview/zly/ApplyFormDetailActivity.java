...
public class ApplyFormDetailActivity extends Activity {
    ...
    public static void startActivity(Context context, int applyId){
    ...
    private void requestData() {
        ...
        Observable.create(new Observable.OnSubscribe<TUserApply>() {
            @Override
            public void call(Subscriber<? super TUserApply> subscriber) {
                int applyId = getIntent().getIntExtra(KEY_APPLY_ID, -1);
                TUserApply userApply = SQLite.select().from(TUserApply.class).where(TUserApply_Table.id.eq(applyId) ).querySingle();
                subscriber.onNext(userApply);
            }
        }).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<TUserApply>() {
                    @Override
                    public void call(TUserApply tUserApply) {
                        updateView(tUserApply);
                    }
